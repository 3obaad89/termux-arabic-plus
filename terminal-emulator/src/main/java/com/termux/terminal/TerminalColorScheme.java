...
/**
 * Color scheme for a terminal with default colors, which may be overridden (and then reset) from
 * the shell using Operating System Control (OSC) sequences.
 *
 * Default theme: Dracula (beautiful dark theme).
 * The 256-color xterm cube is preserved for program compatibility.
 *
 * @see TerminalColors
 */
public final class TerminalColorScheme {

    /** Default theme: Dracula — applied on first load, then the 256-color cube + greyscale ramp follow. */
    private static final int[] DEFAULT_COLORSCHEME = new int[TextStyle.NUM_INDEXED_COLORS];

    static {
        // ── Step 1: 256-color xterm cube (standard, unchanged) ──
        int[] cube = {
            // 16 original colors — filled from Dracula below
